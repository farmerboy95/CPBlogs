...
Có một mẹo rất đơn giản để có xác suất tốt hơn. Ta có thể tính hai hash khác nhau cho mỗi xâu (bằng cách sử dụng hai $p$ khác nhau, và/hoặc $m$ khác nhau, và so sánh các cặp này thay thế. Nếu $m$ khoảng $10^9$ cho cả hai hash function thì nó sẽ gần như tương đương với việc có một hash function với $m \approx 10^{18}$. Khi so sánh $10^6$ xâu với nhau, xác suất ít nhất một va chạm xảy ra bây giờ giảm xuống còn $\approx 10^{-6}$.
